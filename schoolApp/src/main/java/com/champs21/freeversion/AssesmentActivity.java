...
import com.champs21.schoolapp.NotifyService;
...
import com.champs21.schoolapp.viewhelpers.PopupDialogAssessmentNextQuestion;
...
public class AssesmentActivity extends ChildContainerActivity implements View.OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(getIntent().getStringExtra("from_assessment_leaderboard") != null)
		{
			assesmentId = getIntent().getExtras().getString("from_assessment_leaderboard");
			
			if(!TextUtils.isEmpty(assesmentId))
			{
				initApiCallPlayCount();
				initApiCall();
			}
		}
		else
		        	...
		        	else
		        	{
		        		...
		        		if(currentPosition < listAssessmentQuestion.size())
		        		{
							...
							else
							{
								if(currentPosition <= listAssessmentQuestion.size())
								{
									listAssessmentQuestionSummery.add(new PopulateSummeryData(listAssessmentQuestion.get(currentPosition-1).getQuestion(), isValid, 
	...
	private void initApiCallAddmark() {
		...
		params.put("time_taken", String.valueOf(getEllaspsedSeconds()));
		
		
		float avgTime = (float)getEllaspsedSeconds()/totalQuestionNumber;
		DecimalFormat df = new DecimalFormat();
		df.setMaximumFractionDigits(2);
		df.format(avgTime);
		
		params.put("avg_time", String.valueOf(df.format(avgTime)));
		
		Log.e("ADD_MARK_ID", "id: "+UserHelper.getUserFreeId());
	...
	private void populateData(String title, String topic, String time, String playCount)
	{
		this.txtTitle.setText(title);
		this.txtTopic.setText(topic);
		this.txtTime.setText(time);
		this.txtPlayCount.setText(playCount);
				...
				//{
					...
					child.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							currentPosition++;
							if(currentPosition < AssesmentActivity.this.listAssessmentQuestion.size()){
								//populateDataQuestionAfterFirstTime(listAssessmentQuestion);
	...
	private boolean isValidChoosen()
	{
		...
		if(currentPosition-1 < listAssessmentQuestion.size())
		{
			...
			if(isValid)
			{
				score = score + Integer.parseInt(listAssessmentQuestion.get(currentPosition-1).getMark());
			}
	...
	private void showCustomDialogScore(String headerText, int imgResId,
			...
			picker.setData(headerText,descriptionText,imgResId, AssesmentActivity.this);
	...
	private void showCustomDialogNextQuestion(String headerText, int imgResId,
			...
			PopupDialogAssessmentNextQuestion picker = PopupDialogAssessmentNextQuestion.newInstance(0);
			picker.setData(headerText,descriptionText,imgResId, AssesmentActivity.this, answerText, questionText, explanationText, new PopupDialogAssessmentNextQuestion.IButtonclick() {
	...
	private void showCustomDialogOk(final boolean isLoggedIn, boolean isShowRetryButton, String titleText, String scoreText, String description, int iconResId,
			...
			picker.setData(isLoggedIn, isShowRetryButton, titleText, scoreText, description, iconResId, AssesmentActivity.this, new PopupDialogAssessmentOk.OkCallback() {
				...
				public void onSummeryCalled() {
					...
					Gson gson = new Gson();
					Intent intent = new Intent(AssesmentActivity.this, AssessmentSummeryActivity.class);
	...
	public class PopulateSummeryData{
		...
		public PopulateSummeryData()
		{
			
		}
		
		public PopulateSummeryData(String question, boolean isRightAnswer, String answer, String explanation)
		{
